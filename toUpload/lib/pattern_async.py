...
class Pattern:
    ...
    async def get(self,index):  #  Usage: item = await queue.get()
    ...
    def _put(self,index,val):
            self._upd_jnevt(1) # update join event
            self._evput.set()  # Schedule tasks waiting on put
            self._evput.clear()
            self._queue[index] = val

    async def put(self,index, val):  # Usage: await queue.put(item)
        while self.full():
            # Queue full
            await self._evget.wait()
            # Task(s) waiting to get from queue, schedule first Task
